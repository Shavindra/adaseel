...
The current milestone deliberately stops after deterministic flagging. Report type
is metadata, not a hard-coded blood-test domain: a user-supplied type wins, otherwise
MEDLENS uses an explicit label in the document and finally a small deterministic
signature registry. Unsupported or ambiguous inputs remain ``unknown``.
"""
...
SAMPLE_IMG = str(PROJECT_ROOT / "sample_lab_report.png")
SAMPLE_TXT = str(PROJECT_ROOT / "sample_lab_report.txt")
...
def build_flagging_report(
    ...
    *,
    ...
    """Render the optional human-readable output for this intermediate milestone."""
    ...
    content = [
        ...
        "- **Pipeline stop:** deterministic flagging; no evidence research or model reasoning ran",
        "",
    ...
    content += [
        ...
        (
            ...
            if debug_mode
            else "- The trace records rules, explicit rationales, reason codes, timings, "
            "counts, and hashes; it excludes OCR text, exact values, identifiers, "
