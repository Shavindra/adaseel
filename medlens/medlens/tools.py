...
The orchestrator calls these functions directly. Bounded agents emit typed artefacts,
but they do not choose the workflow order, alter canonical rows/reference ranges, or
invoke output writing.
"""
...
# These deterministic tools are not advertised to the bounded language agents.
TOOL_SCHEMAS = [
    {
        "type": "function",
        "function": {
...
def write_flagging_report(ctx: dict[str, Any], out_path: str) -> dict[str, str]:
    ...
    try:
        report = labtools.build_flagging_report(
            ...
            agent_invocations=ctx.get("agent_invocations") or [],
            handoffs=ctx.get("handoffs") or [],
        )
