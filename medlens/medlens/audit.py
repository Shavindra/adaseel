# -*- coding: utf-8 -*-
"""Durable run tracing for deterministic tools and bounded MEDLENS agents.
...
RUN_SCHEMA_VERSION = "medlens.run.v2"
TRACE_SCHEMA_VERSION = "medlens.trace.v2"
...
def _render_human_value(value: Any) -> list[str]:
    """Render one already-redacted value as indented key/value or bullet lines."""
    if value is None:
        return ["null"]
    ...
    if isinstance(value, str):
        if "\n" not in value:
            return [value]
        return ["|"] + ["  %s" % line for line in value.splitlines()]
    if isinstance(value, dict):
        if not value:
            return ["(none)"]
        lines = []
        for key in sorted(value):
            child = _render_human_value(value[key])
            if len(child) == 1:
                lines.append("%s: %s" % (key, child[0]))
            else:
                lines.append("%s:" % key)
                lines.extend("  %s" % line for line in child)
        return lines
    if isinstance(value, (list, tuple)):
        ...
        lines = []
        for item in value:
            child = _render_human_value(item)
            if len(child) == 1:
                lines.append("- %s" % child[0])
            else:
                lines.append("-")
                lines.extend("  %s" % line for line in child)
        return lines
...
def render_human_event(event: dict[str, Any]) -> str:
    """Render one structured event as a scan-friendly, multi-line log entry."""
    ...
    if reason_codes:
        lines.append("  reasons: %s" % ", ".join(str(item) for item in reason_codes))
    details = event.get("details") or {}
    for key in sorted(details):
        rendered = _render_human_value(details[key])
        if len(rendered) == 1:
            lines.append("  %s: %s" % (key, rendered[0]))
            continue
        lines.append("  %s:" % key)
        lines.extend("    %s" % line for line in rendered)
    return "\n".join(lines) + "\n\n"


class TraceWriter:
    """Append-only JSONL and human-readable event writers."""
    ...
    def __init__(
        ...
        self._log_handle.write(
            "MEDLENS human-readable run log\n"
            "run_id: %s\n"
            "structured_event_stream: events.jsonl\n\n" % self.run_id
        )
    ...
    def emit(
        ...
        *,
        ...
        self._log_handle.write(render_human_event(event))
        self._log_handle.flush()
        os.fsync(self._log_handle.fileno())
        return event
